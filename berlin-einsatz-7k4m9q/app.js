(() => {
  ...
  function markerCard(marker, def) {
    ...
    return `<article class="marker-card">${photo}<div><strong>${esc(street)}</strong><span>${Number(marker.lat).toFixed(6)}, ${Number(marker.lng).toFixed(6)}</span><small>±${Math.round(Number(marker.accuracy) || 0)} m · ${esc(marker.driver || 'Fahrer unbekannt')}</small><a href="${map}" target="_blank" rel="noopener">In Google Maps öffnen ↗</a></div></article>`;
  }
